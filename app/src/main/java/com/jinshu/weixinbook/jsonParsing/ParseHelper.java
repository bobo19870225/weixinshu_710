...
//			LogUtil.d(e.toString());
	...
	public static <T extends BaseModel> List getModelList(
			JSONArray jArray, Class<T> clazz) {
		try {
			if (jArray != null) {
...
//			LogUtil.d(e.toString());
	...
	public static <T extends BaseModel> T getModel(JSONObject rootObject, Class<T> clazz) {
		T itemModel;
		try {
			itemModel = clazz.newInstance();
		} catch (InstantiationException e) {
		...
		mParser.parseJSONObject(rootObject, itemModel);
